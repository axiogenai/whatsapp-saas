...
export async function GET(request: Request) {
  ...
  try {
    const res = await fetch(`${GATEWAY_URL}/api/tenant/${tenantId}/config`, {
      cache: 'no-store',
  ...
  return NextResponse.json({ error: 'Failed to fetch config' }, { status: 502 });
}
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ error: `Failed to save: ${err.message}` }, { status: 502 });
  }
}
